...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (path === "/chat/clear" && method === "POST") {
      try {
        const body = await req.json();
...
console.log(`   Max requests per session: ${RATE_LIMIT_CONFIG.MAX_REQUESTS_PER_SESSION}`);
console.log(`   Max requests per IP per minute: ${RATE_LIMIT_CONFIG.MAX_REQUESTS_PER_IP_PER_MINUTE}`);
console.log(`   Max requests per IP per hour: ${RATE_LIMIT_CONFIG.MAX_REQUESTS_PER_IP_PER_HOUR}`);
console.log(`   Global max requests per minute: ${RATE_LIMIT_CONFIG.GLOBAL_MAX_REQUESTS_PER_MINUTE}`);
console.log(`   Max message length: ${RATE_LIMIT_CONFIG.MAX_MESSAGE_LENGTH} characters`);
